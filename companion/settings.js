...
import { mmol, mgdl } from "../common/utils";

export function get(dataReceivedFromWatch) {
	...
	if (settingsStorage.getItem('dataSource')) {
		...
		settingsStorage.setItem("dataSource", JSON.stringify({
			"selected": [0],
			"values": [{
				"name": "Dexcom",
				"value": "dexcom"
	...
	if (dataSource === 'nightscout') { // Nightscout
		...
		if (settingsStorage.getItem('nightscoutSiteName') && JSON.parse(settingsStorage.getItem('nightscoutSiteName')).name) {
			...
			settingsStorage.setItem("nightscoutSiteName", JSON.stringify({
				"name": ''
			}));
		}
		...
		if (settingsStorage.getItem('nightscoutSiteHost')) {
			...
			settingsStorage.setItem("nightscoutSiteHost", JSON.stringify({
				"selected": [0],
				"values": [{
					"name": 'Heroku',
					"value": 'herokuapp.com'
	...
	if (settingsStorage.getItem('glucoseUnits')) {
		...
		glucoseUnits = 'mgdl';
		settingsStorage.setItem("glucoseUnits", JSON.stringify({
			"selected": [0],
			"values": [{
				"name": glucoseUnits,
				"value": glucoseUnits
	...
	if (settingsStorage.getItem('highThreshold')) {
		...
		highThreshold = 180;
		settingsStorage.setItem("highThreshold", JSON.stringify({
			"name": highThreshold
		}));
	}
	...
	if (settingsStorage.getItem('lowThreshold')) {
		...
		lowThreshold = 70;
		settingsStorage.setItem("lowThreshold", JSON.stringify({
			"name": lowThreshold
		}));
	}
	...
	if (settingsStorage.getItem('dismissHighFor')) {
		...
		dismissHighFor = 120;
		settingsStorage.setItem("dismissHighFor", JSON.stringify({
			"name": dismissHighFor
		}));
	}
	...
	if (settingsStorage.getItem('dismissLowFor')) {
		...
		dismissLowFor = 15;
		settingsStorage.setItem("dismissLowFor", JSON.stringify({
			"name": dismissLowFor
		}));
	}
	...
	if (settingsStorage.getItem('timeFormat')) {
		...
		timeFormat = '12hr';
		settingsStorage.setItem("timeFormat", JSON.stringify({
			"selected": [0],
			"values": [{
				"name": timeFormat,
				"value": false
	...
	if (settingsStorage.getItem('dateFormat')) {
		...
		settingsStorage.setItem("dateFormat", JSON.stringify({
			"selected": [0],
			"values": [{
				"name": dateFormat,
				"value": dateFormat
	...
	if (settingsStorage.getItem('tempType')) {
		...
		tempType = 'f';
		settingsStorage.setItem("tempType", JSON.stringify({
			"selected": [0],
			"values": [{
				"name": 'Fahrenheit',
				"value": tempType
	...
	if (settingsStorage.getItem('bgColor')) {
		...
		if (bgColor === '#FFFFFF') {
			...
			if (!saveColor) {
				settingsStorage.setItem("hexColor", JSON.stringify({
					"name": bgColor
				}));
			} else {
	...
	if (settingsStorage.getItem('layoutOne') && JSON.parse(settingsStorage.getItem('layoutOne')).values) {
		...
		layoutOne = 'iob';
		settingsStorage.setItem("layoutOne", JSON.stringify({
			"selected": [0],
			"values": [{
				name: "Insulin on board (default)",
				value: "iob"
			}]
		}));
	}
	...
	if (settingsStorage.getItem('layoutTwo') && JSON.parse(settingsStorage.getItem('layoutTwo')).values) {
		...
		layoutTwo = 'cob';
		settingsStorage.setItem("layoutTwo", JSON.stringify({
			"selected": [0],
			"values": [{
				name: "Carbs on board (default)",
				value: "cob"
			}]
		}));
	}
	...
	if (settingsStorage.getItem('layoutThree') && JSON.parse(settingsStorage.getItem('layoutThree')).values) {
		...
		layoutThree = 'step';
		settingsStorage.setItem("layoutThree", JSON.stringify({
			"selected": [0],
			"values": [{
				name: "steps (default)",
				value: "steps"
	...
	if (settingsStorage.getItem('layoutFour') && JSON.parse(settingsStorage.getItem('layoutFour')).values) {
		...
		layoutFour = 'heart';
		settingsStorage.setItem("layoutFour", JSON.stringify({
			"selected": [0],
			"values": [{
				name: "heart (default)",
				value: "heart"
	...
	if (settingsStorage.getItem('dexcomUsername')) {
		...
		dexcomUsername = null;
		settingsStorage.setItem("dexcomUsername", JSON.stringify({
			"name": dexcomUsername
		}));
	}
	...
	if (settingsStorage.getItem('dexcomPassword')) {
		...
		dexcomPassword = null;
		settingsStorage.setItem("dexcomPassword", JSON.stringify({
			"name": dexcomPassword
		}));
	}
	...
	if (settingsStorage.getItem('staleDataAlertAfter')) {
		...
		settingsStorage.setItem("staleDataAlertAfter", JSON.stringify({
			"name": staleDataAlertAfter
		}));
	}
